  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight uppercase">Tournament Gallery</h1>
          <p className="text-slate-400 text-xs">Explore memorable snapshots captured across matches</p>
        </div>
      </div>

      {/* Filter Row */}
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl flex flex-col sm:flex-row gap-4 items-center justify-between shadow-md">
        <div className="flex items-center gap-2 text-slate-400 text-xs font-semibold uppercase tracking-wider">
          <Filter className="w-4 h-4 text-emerald-500" /> Filter Photos
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredPhotos.map((photo) => (
            <div
              ...
              className="bg-slate-900 border border-slate-800/80 rounded-2xl overflow-hidden shadow-lg cursor-pointer hover:border-emerald-500/20 transition-all hover:-translate-y-0.5 group relative aspect-video"
            >
              <img
                src={photo.image_url}
              ...
              <div className="absolute top-2 left-2">
                {photo.match ? (
                  <span className="bg-slate-950/80 backdrop-blur px-2.5 py-0.5 rounded-lg border border-slate-800 text-[9px] font-extrabold text-emerald-400 uppercase tracking-wider shadow">
                    {photo.match.team1?.short_name} vs {photo.match.team2?.short_name}
                  </span>
                ) : (
                  <span className="bg-slate-950/80 backdrop-blur px-2.5 py-0.5 rounded-lg border border-slate-800 text-[9px] font-extrabold text-slate-400 uppercase tracking-wider shadow">
                    Tournament
                  </span>
                )}
              </div>
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/40 to-transparent opacity-0 group-hover:opacity-100 flex flex-col justify-end p-4 transition-opacity duration-200">
                <span className="bg-emerald-500 text-slate-950 font-black text-[9px] px-2 py-1 rounded-md self-center mb-auto shadow flex items-center gap-1">
